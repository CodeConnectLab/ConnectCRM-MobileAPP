...
const FilterModel = ({
  ...
  const HandleUpdate = type => {
    if (type === 'Clear') {
      ...
      onUpdate({
        ...
        FormData: formState.FromDate || null,
        ToData: formState.ToDate || null,
      });
    }
  };
    ...
    <View
      ...
      <View
        ...
        <View style={{flexDirection: 'row', gap: 10}}>
          <Pressable
            onPress={() =>
              setFocusStates(prev => ({...prev, ['FromDate']: true}))
            }
            style={{
              ...styles.boxDrop,
              marginTop: 5,
            ...
            <Text
              style={{
                ...
                justifyContent: 'center',
                fontSize: 15,
                color: formState.FromDate ? COLORS.Black : COLORS.LightGray,
              }}>
              {formState?.FromDate
                ? formState?.FromDate?.toDateString() + ''
                : 'form Date'}
            </Text>
          </Pressable>
          <Pressable
            onPress={() =>
              setFocusStates(prev => ({...prev, ['ToDate']: true}))
            }
            style={{
              ...styles.boxDrop,
              height: 40,
            ...
            <Text
              style={{
                ...
                justifyContent: 'center',
                fontSize: 15,
                color: formState.ToDate ? COLORS.Black : COLORS.LightGray,
              }}>
              {formState.ToDate ? formState?.ToDate?.toDateString() : 'To Date'}
            </Text>
          </Pressable>
        </View>
        <View
  ...
  return (
    <View style={{}}>
      ...
      <DatePicker
        modal
        open={focusStates['FromDate']}
        date={formState?.FromDate || new Date()}
        mode="date" // 'date', 'time', or 'datetime'
        onConfirm={selectedDate => {
          setFocusStates(prev => ({...prev, ['FromDate']: false}));
      ...
      <DatePicker
        modal
        open={focusStates['ToDate']}
        date={formState?.ToDate || new Date()}
        mode="date" // 'date', 'time', or 'datetime'
        onConfirm={selectedDate => {
          setFocusStates(prev => ({...prev, ['ToDate']: false}));
          handleInputChange('ToDate', selectedDate);
        }}
        onCancel={() => setFocusStates(prev => ({...prev, ['ToDate']: false}))}
      />
    </View>
  );
};
